...
def plot_portfolio_allocation(weights, portfolio_name):
    ...
    filtered_weights = {
        asset: weight for asset, weight in weights.items() if weight > 0
    }
...
def optimize_portfolio(
    portfolio_name, time_constraint=None, asset_constraints=None, geo_exposure=None
):
    try:
        portfolio_price_data = pd.read_csv(
            f"./data/exports/{portfolio_name}_collated.csv",
            parse_dates=True,
            index_col="Date",
        )
    ...
    if portfolio_price_data.empty:
        print(
            f"Error: No data available for {portfolio_name} after applying time constraint. Skipping optimization."
        )
        return None, None

    if portfolio_price_data.isnull().values.any():
        print(
            f"Warning: Missing data found in {portfolio_name}. Filling missing values with forward fill."
        )
    ...
    print(
        f"Predicted performance for {portfolio_name} (Expected Annual Return, Annual Volatility, Sharpe Ratio): \n {performance}"
    )
    ...
    for portfolio in portfolios:
        try:
            portfolio_data = pd.read_csv(
                os.path.join(portfolio_dir, f"{portfolio}.csv")
            )
