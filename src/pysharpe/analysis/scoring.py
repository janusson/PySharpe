...
DEFAULT_TECH_WEIGHTS = {"P_YL": 0.3, "P_SMA200": 0.2, "P_ML3": 0.2, "PER": 0.3}
...
def composite_score(
    tech_score: float, div_score: float, alpha: float = 0.5, beta: float = 0.5
) -> float:
    """
    ...
    if tech_score + div_score > 1.0:
        raise ValueError(
            "Combined scores must not exceed 1.0 to preserve the weighted scale."
        )
